...
public class RecFurnaceTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider
{
	...
	/**
	 * this is what is displayed as the name of the Block in the GUI
	 * @return
	 */
	private ITextComponent getDefaultName()
	{
		return new TranslationTextComponent("Powered Furnace");
	}
	...
	public void read(BlockState stateIn, CompoundNBT compound)
	{
		...
		if(compound.contains(NBTConstants.NAME, Constants.NBT.TAG_STRING))
		{
			//this.customName = ITextComponent.Serializer.func_240643_a_(compound.getString("CustomName"));
			this.customName = ITextComponent.Serializer.getComponentFromJson(compound.getString(NBTConstants.NAME));
		}
		...
		this.currentSmeltTime = compound.getInt(NBTConstants.SMELT_TIME);
	}
	...
	public CompoundNBT write(CompoundNBT compound)
	{
		...
		{
			compound.putString(NBTConstants.NAME, ITextComponent.Serializer.toJson(this.customName));
		}
		
		ItemStackHelper.saveAllItems(compound, this.inventory.toNonNullList());
		compound.putInt(NBTConstants.SMELT_TIME, this.currentSmeltTime);
		return compound;
	}
